...
> Template — copy to `{slice-id}-{kebab-title}.md` and fill in. `make bootstrap-slice SLICE={ID}` does this automatically.

| Field | Value |
|---|---|
| **Track** | A / B / C |
| **Branch** | `track-{x}/{ID}-{kebab-title}` |
| **Worktree** | `~/ares-track-{x}/{ID}` |
| **PR** | TBD |
